...
from ..scenario_manager.scenario_manager_noBEV_scenarioType import ScenarioManager
...
class CarPLANPlanner(AbstractPlanner):
    ...
    def compute_planner_trajectory(
        ...
        planning_trajectory = self._run_planning_once(current_input, scenario_token, scenario_type, scenario)

        self._inference_runtimes.append(time.perf_counter() - start_time)

        return planning_trajectory

    def _run_planning_once(self, current_input: PlannerInput, scenario_token, scenario_type, scenario):
        ego_state = current_input.history.ego_states[-1]

        planner_feature = self._planner_feature_builder.get_features_from_simulation(
            current_input, self._initialization, scenario_type
        )
        ...
        if candidate_trajectories.shape[0] == 0:
            trajectory = self._local_to_global(ref_free_trajectory, ego_state)
