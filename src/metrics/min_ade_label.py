...
class minADE_label(Metric):
    ...
    def __init__(
        ...
        super(minADE_label, self).__init__(
    ...
    def update(self, outputs: Dict[str, torch.Tensor], target: torch.Tensor) -> None:
        if outputs["label_trajectory"] is not None:
            with torch.no_grad():
                pred, _ = sort_predictions(
                    outputs["label_trajectory"], outputs["label_probability"], k=self.k
                )
                ade = torch.norm(
