...
class RouteManager:
    ...
    def load_route(self, ego_state: EgoState, process=True) -> None:
        ...
        self._route_roadblock_dict = {} #JY Route에 해당하는 RoadBlock 정보
        self._route_lane_dict = {} #JY #Route에 해당하는 RoadBlock 안에 있는 lane 정보
    ...
    def get_ego_path(self, ego_state: EgoState, search_depth=15):
        current_lane = self._get_starting_lane(ego_state) #JY #현재 위치에 있는 current lane
        ...
        route_plan, path_found = graph_search.search(roadblock_window[-1]) #JY 현재 위치에서 search_depth까지의 최단 거리 lane 정보 모음
        ...
        return centerline_discrete_path #JY 현재 위치에서 search_depth까지의 최단 거리에 해당하는 모든 point 좌표
    ...
    def get_reference_lines(self, ego_state: EgoState, interval=1.0, length=100):
        ...
        for lane in self._get_candidate_starting_lane(ego_state):
            candidate_discrete_path, candidate_discrete_polygon = self.find_all_candidate_routes(ego_state, lane, maximum_length=length)
            discrete_paths.extend(candidate_discrete_path)
            discrete_polygons.extend(candidate_discrete_polygon)
        ...
        if length_mask.any() and not length_mask.all():
            ...
            discrete_polygons = [discrete_polygons[i] for i in np.where(length_mask)[0]]
        ...
        merged_polygons = [
            discrete_polygons[i] for i in range(len(discrete_polygons)) if i not in remove_index
        ]
        self.reference_lines = merged_paths

        return merged_paths, merged_polygons
    ...
    def find_all_candidate_routes(
        ...
        for lane_list in candidate_route:
            ...
            for lane in lane_list:
                ...
                discrete_polygon.append(lane.polygon)
                length += lane.baseline_path.length
            candidate_discrete_path.append(discrete_path)
            candidate_discrete_polygon.append(discrete_polygon)

        return candidate_discrete_path, candidate_discrete_polygon
