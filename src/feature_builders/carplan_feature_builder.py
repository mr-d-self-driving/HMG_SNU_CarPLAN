...
from src.scenario_manager.cost_map_manager_noBEV_scenarioType import CostMapManager
from src.scenario_manager.scenario_manager_noBEV_scenarioType import OccupancyType, ScenarioManager
from . import common
...
class CarPLANFeatureBuilder(AbstractFeatureBuilder):
    def __init__(
        self,
        radius: float = 100,
        ...
        is_make_extra_res: bool = False,
        is_front2rear: bool = False,
    ) -> None:
        super().__init__()
    ...
    def get_feature_type(self) -> Type[AbstractModelFeature]:
        ...
        return CarPLANFeature  # type: ignore

    def get_class(self) -> Type[AbstractFeatureBuilder]:
        return CarPLANFeatureBuilder
    ...
    def get_features_from_simulation(
        self, current_input: PlannerInput, initialization: PlannerInitialization, scenario_type="default"
    ) -> AbstractModelFeature:
    ...
    def _build_feature(
        ...
        present_ego_state = ego_state_list[present_idx] #JY 현재 ego state
        query_xy = present_ego_state.center #JY AV의 center 축(axle) 좌표
        traffic_light_status = list(traffic_light_status)  # note: tl is a iterator #JY #각 레인 id의 신호등 상태정보
        
        #JY
        self._update_base_info(present_ego_state)
    
        if self.scenario_manager is None:
            scenario_manager = ScenarioManager(
                map_api,
                present_ego_state,
                route_roadblocks_ids, #JY #목적지까지의 route block id 정보
                radius=50,
            )
            scenario_manager.update_ego_state(present_ego_state)
            scenario_manager.update_drivable_area_map() #JY 50반경 내에 있는 roadblock, roadblockConnector, CarPark 정보 업데이트
            
        else:
            scenario_manager = self.scenario_manager
        
        #JY
        route_roadblocks_ids = scenario_manager.get_route_roadblock_ids() #JY Route에 해당하는 roadBlock id 정보 #시각화 필요
        route_reference_path, route_centerline = scenario_manager.update_ego_path(length=self.radius) #JY 현재 위치에서 목적지까지의 최단 거리 lane 정보 (최대 50m로 자르기)
        red_light_in_route_polygon = scenario_manager.update_obstacle_map( #JY 50m 이내의 Dynamic & Static이나 route 안에 레인이 빨간불일 경우 장애물로 판단
        ...
        #JY
        # data["centerline"] = self._get_centerline_feature(scenario_manager=scenario_manager, route_centerline=route_centerline)

        data["current_state"] = self._get_ego_current_state( #JY x,y,heading,vel,acc,steering_angle,yaw_rate
            ego_state_list[present_idx], ego_state_list[present_idx - 1]
        )

        ego_features = self._get_ego_features(ego_states=ego_state_list) #JY 101개 타임스텝에 대한 각종 state 정보 (rearAxle 기준)state 정보
        agent_features, agent_tokens, agents_polygon = self._get_agent_features( #JY 101개 타임스텝에 대한 최대 주변 48개 agent들의 각종 state 정보
            query_xy=query_xy, 
            present_idx=present_idx,
            tracked_objects_list=tracked_objects_list,
        )

        data["agent"] = {} #JY ego 포함
        ...
        data["map"], map_polygon_tokens = self._get_map_features( #JY #반경 120m내의 Lane, Lane_connector, Crosswalk에 대한 정보 추출
            map_api=map_api,
            query_xy=query_xy,
        ...
        if self.build_reference_line:
            data["reference_line"], merged_polygons = self._get_reference_line_feature(
        ...
        return CarPLANFeature.normalize(data, first_time=True, radius=self.radius)
    ...
    def scenario_casual_reasoning_preprocess(
        ...
        leading_objects = scenario_manager.get_leading_objects() #JY 목적지까지의 50m 이내 최단거리 path에 존재하는 장애물 정보
    ...
    def _get_agent_features(
        ...
        agent_ids_sorted = agent_ids[np.argsort(distance)[: self.max_agents]] #JY 거리순으로 최대 48개의 주변 agent 정보 가져오기
    ...
    #JY
    def _update_base_info(self, present_ego_state):
        self.origin = present_ego_state.rear_axle.array
        angle = present_ego_state.rear_axle.heading
    ...
    def _get_reference_line_feature(
        ...
        reference_lines, merged_polygons = scenario_manager.get_reference_lines(length=self.radius)
    ...
    #JY
    def _get_centerline_feature(
        self, scenario_manager: ScenarioManager, route_centerline
    ):
        path, path_len = scenario_manager._route_manager._trim_discrete_path(scenario_manager._ego_state, route_centerline, self.radius)
        ...
        valid_mask = np.zeros((1, n_points), dtype=np.bool)

        subsample = path[::4][: n_points + 1]
        n_valid = len(subsample)
        position[0, : n_valid - 1] = subsample[:-1, :2]
        vector[0, : n_valid - 1] = np.diff(subsample[:, :2], axis=0)
        orientation[0, : n_valid - 1] = subsample[:-1, 2]
