...
class SimulationRunner(AbstractRunner):
    ...
    def run(self) -> RunnerReport:
        ...
        for _ in tqdm(range(self.simulation.scenario.get_number_of_iterations()), desc=f"{self.simulation.scenario.token} 시뮬레이션 중..."): #JY
            if self.simulation.is_simulation_running():
                ...
                #JY
                scenario_token = self.simulation.scenario.token
                scenario_type = self.simulation.scenario.scenario_type
                trajectory = self.planner.compute_trajectory(planner_input, scenario_token, scenario_type, self.simulation.scenario)
